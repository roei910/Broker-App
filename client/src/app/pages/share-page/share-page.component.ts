...
export class SharePageComponent {
  ...
  getStock() {
    var url = `https://localhost:7072/api/Shares/GetShare?ShareId=${this.stockId
      }`;
    this.http.get<any>(url).subscribe((data) => {
      if (data.statusCode == 404)
  ...
  getStockTransactions() {
    var url = `https://localhost:7072/api/Shares/Share10Transactions?stockId=${this.stockId
      }`;
    this.http.get<any>(url).subscribe((data) => {
      if (data.statusCode == 404)
        alert('Problem fetching share from server');
  ...
  getOpenRequests() {
    var url = `https://localhost:7072/api/Shares/GetShareOpenRequests?stockId=${this.stockId
      }`;
    this.http.get<any>(url).subscribe((data) => {
      if (data.statusCode == 404)
        alert('Problem fetching share from server');
